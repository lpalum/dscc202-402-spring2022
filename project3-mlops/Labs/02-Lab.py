...
# TODO
artifactURI = 'runs:/'+run.info.run_id+"/grid-random-forest-model"
...
with mlflow.start_run(run_name="RF-Grid-Search-2") as run:
    ...
    predictions = best_rf.predict(X_test)
    # Log model with name
    mlflow.sklearn.log_model(best_rf, "grid-random-forest-model-2")
    # Log params
    mlflow.log_params(best_rf.get_params())
    # Create and log MSE metrics using predictions of X_test and its actual value y_test
    mse = mean_squared_error(y_test, predictions)
...
# Get error of most recent run
client.search_runs(run.info.experiment_id, order_by=["attributes.start_time desc"], max_results=1)[0].data.metrics
